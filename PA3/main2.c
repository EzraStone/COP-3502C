...
void mypermute(int arr[], int used[], int k, int n){
    if (k == n){ // Checks if the length has been reached
        if(isValid(arr, n)){ // Checks if it is a valid permutation
            printPermutArray(arr, n); // Prints the permutations
            exit(0); // Quits program once the first permutation has been made cause the others are not needed    
        }
    }
    ...
    for (int i = 0; i < n; i++){
        if (!used[i]){ // Checks if i was not used
            ...
            mypermute(arr, used, k + 1, n);// Pass in our new arr and used and increment k
...
int main(){
    ...
    mypermute(array, used, 0, num_of_people);// Call my permute 
